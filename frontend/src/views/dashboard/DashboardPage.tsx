  ...
  return (
    <div className="p-8 lg:p-10">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-indigo-600">Workspace overview</p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight">Dashboard</h1>
          <p className="mt-2 text-sm text-slate-500">Track projects, test coverage and the latest execution status from one place.</p>
        </div>
        <Link to="/projects" className="inline-flex h-10 items-center gap-2 rounded-lg bg-indigo-600 px-4 text-sm font-medium text-white hover:bg-indigo-700">
          <Plus size={17} /> Manage Projects
        </Link>
      </div>

      {error && <div className="mt-6 rounded-lg-lg-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Could not refresh dashboard data: {error}</div>}
        ...
        {cards.map(({ label, value, icon: Icon, helper }) => (
          <Card key={label} className="p-5">
            <div className="flex items-start justify-between">
              ...
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-50 text-indigo-600"><Icon size={19} /></span>
            </div>
      ...
      <div className="mt-9 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Open a project to review its suites and current test status.</p>
        </div>
        <Link to="/projects" className="flex items-center gap-1 text-sm font-semibold text-indigo-600 hover:text-indigo-700">View all <ArrowRight size={15} /></Link>
      </div>

      <Card className="mt-4 overflow-hidden">
        {data.recentProjects.length === 0 && !loading ? (
          <div className="px-6 py-12 text-center">
            <FolderKanban className="mx-auto text-slate-300" size={34} />
            <h3 className="mt-3 font-semibold">No projects yet</h3>
            <p className="mt-1 text-sm text-slate-500">Create your first project to start organizing tests.</p>
            <Link to="/projects" className="mt-4 inline-flex text-sm font-semibold text-indigo-600">Go to Projects</Link>
          </div>
        ) : (
              ...
              <tbody>
                {data.recentProjects.map((project) => (
                  <tr className="border-t border-slate-100" key={project.id}>
                    ...
                    <td className="px-5">{project.suites}</td>
